...
cleaned_data <- data %>%
  ...
  # Remove outliers from the response_time column
  filter(response_time >= (quantile(response_time, 0.25) - 1.5 * IQR(response_time)) & 
         response_time <= (quantile(response_time, 0.75) + 1.5 * IQR(response_time))) %>%
